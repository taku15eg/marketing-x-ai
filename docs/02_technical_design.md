...
[Step 2] Vision + DOM分析
    ...
    │
    ▼
[Step 2.5] ルールベース法令一次チェック（compliance-rules.ts）
    │  ・DOMテキストに対して薬機法・景表法の禁止/要注意/要確認表現を正規表現で検出
    │  ・severity（prohibited/caution/review_recommended）を付与
    │  ・human_review_required フラグを設定
    │  ・LLM呼び出し前に実行（LLMへのコンテキスト情報としても提供）
    │
    ▼
[Step 3] 診断
    ...
    │  ・改善仮説を生成
    │  ・Step 2.5の一次チェック結果をClaude APIへコンテキストとして送信
    │
    ▼
[Step 4] 依頼パック生成 + 法令チェックマージ
    │  ・デザイナー/エンジニア向け構造化依頼書
    │  ・計測設計（どこにイベントを仕込むか）
    │  ・ルールベース結果とLLM結果をマージ（source: rule/llm/both）
    │  ・免責事項を付与（法的助言ではない旨を明示）
    │
    ▼
